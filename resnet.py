...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, widths=[64, 128, 256, 512], output_size=1000, zero_init_residual=False, norm_layer=None, init_stride=2):
        ...
        self.layer1 = self._make_layer(block, widths[0], layers[0], norm_layer=norm_layer)
        self.layer2 = self._make_layer(block, widths[1], layers[1], stride=2, norm_layer=norm_layer)
        self.layer3 = self._make_layer(block, widths[2], layers[2], stride=2, norm_layer=norm_layer)
        self.layer4 = self._make_layer(block, widths[3], layers[3], stride=2, norm_layer=norm_layer)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc = nn.Linear(widths[3] * block.expansion, output_size)
